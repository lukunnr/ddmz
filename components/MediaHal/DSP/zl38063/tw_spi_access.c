...
int tw_upload_dsp_firmware(int mode)
{
    ...
    if ((mode == 0) || (mode == 1)) {
        ...
        if (status != VPROC_STATUS_SUCCESS) {
            DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfHbiBoot()\n", status);
        ...
        if (status != VPROC_STATUS_SUCCESS) {
            DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfSaveImgToFlash()\n", status);
            // VprocTwolfHbiCleanup();
            return status;
        }
        ...
        if (status != VPROC_STATUS_SUCCESS) {
            DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfFirmwareStart()\n", status);
            // VprocTwolfHbiCleanup();
            return status;
        }
    }
    ...
    if ((mode == 0) || (mode == 2)) {
        ...
        if (status != VPROC_STATUS_SUCCESS) {
            DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfLoadConfig()\n", status);
            // VprocTwolfHbiCleanup();
            return status;
        }
        ...
        if (status != VPROC_STATUS_SUCCESS) {
            DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfSaveCfgToFlash()\n", status);
            // VprocTwolfHbiCleanup();
            return status;
        }
    ...
    if (status != VPROC_STATUS_SUCCESS) {
        DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfReset()\n", status);
        // VprocTwolfHbiCleanup();
        return status;
    }
#endif
...
// int tw_set_vol(int vol)
// {
//     int status = 0;
//     status = VprocTwolfSetVolume(vol);
//     if (status != VPROC_STATUS_SUCCESS) {
//         DEBUG_LOGE(TAG_SPI, "Error %d:VprocTwolfSetVolume()\n", status);
//         // VprocTwolfHbiCleanup();
//         return -1;
//     }
//     ESP_LOGI(TAG_SPI, "Device boot loading completed successfully0...\n");
//     return 0;
// }
...
int zl38063_comm(int argc, char **argv)
{
    ...
    if ((strcmp(argv[1], "-wr") == 0) || (strcmp(argv[1], "-rd") == 0)) {
        ...
        unsigned short image_num = (unsigned short)strtoul(argv[2], NULL, 0);
        /*Load ZL380x0 firmware from flash*/
        //to do need fix
        //status  = VprocTwolfLoadFwrFromFlash(image_num);
        if (status != VPROC_STATUS_SUCCESS) {
